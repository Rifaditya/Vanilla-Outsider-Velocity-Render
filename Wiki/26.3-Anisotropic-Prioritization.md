...
| **Java Implementation** | `net.vanillaoutsider.velocityrender.math.AnisotropicDistanceHelper` |
| **Client Polling Mixin** | `net.vanillaoutsider.velocityrender.client.mixin.SectionTaskDynamicQueueMixin` |
...
| **Controlling GameRules** | `velocityrender:enabled`, `velocityrender:forward_lead_multiplier`, `velocityrender:min_speed_threshold_pct` |
...
### 3D Pitch-Aware Lookahead & Vertical Section Biasing (BL-VR-002)

![3D Pitch-Aware Lookahead Diagram](../Doc/Media/vertical_lookahead.png)

High-speed movement rarely occurs strictly along a flat 2D plane. When diving steeply with an Elytra ($v_y < -0.50\text{ b/t}$) or firing a firework rocket vertically upward ($v_y > +0.50\text{ b/t}$), players cross vertical sub-chunk boundaries ($16\times 16\times 16$ sections) at speeds that outpace vanilla radial meshing:

1. **3D Flight Vector Normalization**:
   - `ClientVelocityTracker` calculates the full 3D normalized vector component $\hat{v}_y = v_y / s$ and caches it in thread-safe volatile storage (`cachedNormDy`).
2. **Steep Vertical Dives ($v_y < -0.50\text{ b/t}$)**:
   - The dot-product calculation evaluates vertical distance offset $\Delta y = (P_{\text{section}, y} + 0.5) - P_{\text{cam}, y}$.
   - Sub-chunk sections located $Y - 4$ ahead in the dive trajectory receive massive negative distance biases, placing them at the immediate front of `SectionTaskDynamicQueue`.
   - Prevents ground collision pop-in and bedrock void stalls during high-altitude Elytra dives.
3. **Rocket Climbs & Ascents ($v_y > +0.50\text{ b/t}$)**:
   - Sections $Y + 4$ above the player are prioritized, rendering mountain peaks, overhangs, and skybox structures instantly.
4. **Zero-Allocation Hot Path**:
   - Polling checks execute directly on worker render threads with zero object instantiation or lock contention.
